  ...
  return {
    ...
    _hogeId_string: (val0: string) => {
      ...
      return {
        entries_json: {
          $get: (option?: { config?: RequestInit }) =>
            send<Methods2['get']['resBody']>(f, GET, prefix, `${prefix0}${PATH1}`, 'json', option),
        ...
        test_4: {
          ...
          fuga_aa: {
            $get: (option: { query: Methods5['get']['query'], config?: RequestInit }) =>
              send<Methods5['get']['resBody']>(f, GET, prefix, `${prefix0}${PATH3}`, 'json', option),
            $post: (option: { body?: Methods5['post']['reqBody'], query: Methods5['post']['query'], config?: RequestInit }) =>
              send<Methods5['post']['resBody']>(f, POST, prefix, `${prefix0}${PATH3}`, 'json', option),
            $put: (option: { query: Methods5['put']['query'], config?: RequestInit }) =>
              send<Methods5['put']['resBody']>(f, PUT, prefix, `${prefix0}${PATH3}`, 'json', option),
            $delete: (option: { body: Methods5['delete']['reqBody'], query: Methods5['delete']['query'], config?: RequestInit }) =>
              send<Methods5['delete']['resBody']>(f, DELETE, prefix, `${prefix0}${PATH3}`, 'json', option),
          ...
          $put: (option?: { query?: Methods3['put']['query'], config?: RequestInit }) =>
            send<Methods3['put']['resBody']>(f, PUT, prefix, `${prefix0}${PATH2}`, 'json', option),
          $delete: (option: { query: Methods3['delete']['query'], config?: RequestInit }) =>
            send<Methods3['delete']['resBody']>(f, DELETE, prefix, `${prefix0}${PATH2}`, 'json', option),
