...
const parseObject = (text: string): { value: string; length: number } => {
  ...
  while (cursor < length) {
    ...
    const ignoredLength = countIgnored(text.slice(cursor))
    value += text.slice(cursor, cursor + ignoredLength)
    cursor += ignoredLength
...
const parseProp = (
  ...
  const prop: Prop = { value: '', hasQuestion: name.hasQuestion, doc: doc?.value }
...
const parseMethod = (text: string): { value: Method; length: number } => {
  ...
  return {
    value: { name: methodName.value as LowerHttpMethod, props, doc: doc?.value },
    length: cursor
  }
}
...
export const parse = (
  text: string,
  name: string
): { methods: Method[]; doc?: string; $textForApiTypes: string } | null => {
  ...
  return methods.length
    ? {
        methods,
        doc: /\/\*\*[\s\S]+\*\/$/.test(d) ? parseDoc(docText)?.value : undefined,
