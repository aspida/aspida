  ...
  return {
    pet: {
      _petId: (val0: number | string) => ({
        ...
        post: (option: { data: Methods1['post']['reqData'], config?: U }) =>
          client.fetch<void>(prefix, `/pet/${val0}`, 'POST', option, 'URLSearchParams').send(),
        $post: async (option: { data: Methods1['post']['reqData'], config?: U }) =>
          (await client.fetch<void>(prefix, `/pet/${val0}`, 'POST', option, 'URLSearchParams').send()).data,
        delete: (option: { headers: Methods1['delete']['reqHeaders'], config?: U }) =>
          client.fetch<void>(prefix, `/pet/${val0}`, 'DELETE', option).send(),
        $delete: async (option: { headers: Methods1['delete']['reqHeaders'], config?: U }) =>
      ...
      post: (option: { data: Methods0['post']['reqData'], config?: U }) =>
        client.fetch<void>(prefix, '/pet', 'POST', option).send(),
      $post: async (option: { data: Methods0['post']['reqData'], config?: U }) =>
        (await client.fetch<void>(prefix, '/pet', 'POST', option).send()).data,
      put: (option: { data: Methods0['put']['reqData'], config?: U }) =>
        client.fetch<void>(prefix, '/pet', 'PUT', option).send(),
      $put: async (option: { data: Methods0['put']['reqData'], config?: U }) =>
    ...
    user: {
      _username: (val2: number | string) => ({
        ...
        put: (option: { data: Methods9['put']['reqData'], config?: U }) =>
          client.fetch<void>(prefix, `/user/${val2}`, 'PUT', option).send(),
        $put: async (option: { data: Methods9['put']['reqData'], config?: U }) =>
