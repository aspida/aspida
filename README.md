...
;(async () => {
  // To enable
  mock.enableLog()
  await axios.get('/foo', { baseURL: 'https://example.com/api', params: { bar: 'baz' } }) // stdout -> [mock] get: /api/foo/?bar=baz => 200
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td></td>
      <td>
        Generate <code>$route.js</code> or <code>$route.ts</code> required for
    ...
    <tr>
      ...
      <td>
        ...
        Regenerate <code>$route.js</code> or <code>$route.ts</code> according to
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        If multiple directories are specified, <code>$route.js</code> or
        <code>$route.ts</code> is generated in each directory.
      </td>
    </tr>
    <tr>
